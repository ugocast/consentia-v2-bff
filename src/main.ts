...
console.log('SUPABASE_KEY:', process.env.SUPABASE_KEY ? 'Definido' : 'No definido');
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API para la gestión de consentimientos y políticas legales')
  ...
  console.log(`Aplicación ${appConfig.name} v${appConfig.version} iniciada en el puerto ${port}`);
  console.log(`Documentación de la API disponible en: http://localhost:${port}/${appConfig.server.docsPath}`);
}
bootstrap();
