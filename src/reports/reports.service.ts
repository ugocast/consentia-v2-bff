...
export class ReportsService {
   ...
   */
  async generateConsentReport(queryDto: ConsentReportQueryDto): Promise<ConsentReportResultDto> {
    try {
      const { companyId, legalPolicyId, status, startDate, endDate, page = 1, pageSize = 10 } = queryDto;
      ...
      if (error) {
        this.logger.error(`Error al generar reporte de consentimientos: ${error.message}`, error);
        throw new Error(`Error al generar reporte de consentimientos: ${error.message}`);
      }
   ...
   */
  async generateAuditReport(queryDto: AuditReportQueryDto): Promise<AuditReportResultDto> {
    try {
      const {
        companyId,
        userId,
      ...
      if (error) {
        this.logger.error(`Error al generar reporte de auditoría: ${error.message}`, error);
        throw new Error(`Error al generar reporte de auditoría: ${error.message}`);
      }
  ...
  async generateMetrics(queryDto: MetricsQueryDto): Promise<MetricsResultDto> {
    try {
      const { companyId, startDate, endDate, groupBy = MetricsGroupBy.MONTH } = queryDto;
      ...
      if (error) {
        this.logger.error(`Error al obtener consentimientos para métricas: ${error.message}`, error);
        throw new Error(`Error al obtener consentimientos para métricas: ${error.message}`);
      }
      ...
      const consentsByStatus = Object.values(ConsentStatus).map(status => {
        const count = consents.filter(consent => consent.status === status).length;
        return {
          status,
          count,
      ...
      // Calcular tasas
      const grantedCount = consentsByStatus.find(item => item.status === ConsentStatus.GRANTED)?.count || 0;
      const revokedCount = consentsByStatus.find(item => item.status === ConsentStatus.REVOKED)?.count || 0;
      
      const acceptanceRate = totalConsents > 0 ? Math.round((grantedCount / totalConsents) * 100) : 0;
      const revocationRate = grantedCount > 0 ? Math.round((revokedCount / grantedCount) * 100) : 0;
      ...
      if (groupBy === MetricsGroupBy.STATUS) {
        ...
        const timeMap = new Map<string, { total: number; granted: number; denied: number; revoked: number }>();
        ...
        consents.forEach(consent => {
          ...
          if (!timeMap.has(timeKey)) {
            timeMap.set(timeKey, { total: 0, granted: 0, denied: 0, revoked: 0 });
          }
